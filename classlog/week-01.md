...
## 2026-08-30 · 학생 수준 관찰 — 출발점이 예상보다 명확하다

Codex(또는 CLI 에이전트류)를 써본 학생은 거의 없다. 대신 **ChatGPT 웹에서 코드를
작성시켜 복사해 와서 프로젝트를 만드는 방식**에는 이미 익숙한 학생들이 있다 —
1주차 덱이 가정한 "AI 완전 처음"보다는 반 발짝 앞서 있고, 정확히 우리 커리큘럼의
"바퀴를 손으로 돌려봤다" 단계에 와 있는 셈이다. 2주차 Codex 도입("그 바퀴를 AI가
직접 돌린다")이 이 경험과 정확히 맞물리므로, 개념 설명은 줄이고 전환 체감 실습으로
빠르게 진행한다.

## 2026-08-31 · 반 명칭 정리 — class1/class2 확정, 수업시간 변경

명단 이동·추가로 "고등부/중등부"가 실제 구성과 어긋나서 중립적 이름으로 정리했다.
**class1 = 1교시 13:30–15:30 (구 high, 7명) · class2 = 2교시 16:00–18:00 (구 mid, 6명)**.
서버 전 계층(roster·Gitea 조직/저장소·위키·Notion 라벨·Slack 알림·허브·계정 카드)을
일괄 변경하고 파이프라인 재실행으로 검증. 커리큘럼·수업 문서의 "중등부/고등부" 표현은
추후 각 문서 개정 시 정리하기로 했다(기록 문서는 당시 명칭 유지).

## 2026-09-01 · 인프라 점검에서 crond 타임존 드리프트 발견·수정

**증상**: 일요일 20시로 걸어둔 위키 자동화 체인이 실제로는 **월요일 05시**에 실행됐다.
수업 시간(13~18시) 분당 토큰 수집도 그 시간엔 15분 간격만 돌고, 분당 창은 밤 10시~새벽
3시에 열렸다 — 정확히 +9시간, KST-UTC 오프셋.

**원인**: crond 는 시작 시점의 타임존을 캐시한다. 이 서버는 crond 가 2026-04-13 에 뜬 뒤
`/etc/localtime` 이 2026-05-09 에 Asia/Seoul 로 바뀌어, crond 만 옛 타임존(UTC)으로
스케줄을 해석하고 있었다. `timedatectl` 은 KST 정상이고 Python 스크립트들은 호출 시점에
TZ 를 읽어 영향이 없었다 — **crond 단독 드리프트**라 겉으로 드러나지 않았다.

**놓칠 뻔한 2차 피해**: 월요일에 실행되면 `week_of()` 가 None(월요일은 수업일이 아님)을
반환해 옛 날짜-나눗셈 폴백으로 넘어가는데, 폴백은 추석 휴강을 모른다. 그대로 뒀으면
**10월 4일(5주차)부터 위키·Notion·Slack 의 주차 번호가 전부 +1 씩 밀려** 마지막 주가
w13 으로 기록될 뻔했다.

**조치·검증**: `systemctl restart crond`. 시각 지정 임시 cron(`07 11 * * *`)을 넣어
11:07:01 KST 에 정확히 발화하는 것을 확인하고 즉시 제거했다 — 고치기 전이라면 같은
스케줄이 저녁 8시에나 걸렸을 것이다.

**교훈**: 타임존을 바꾸면 crond 를 반드시 재시작한다. 그리고 "cron 이 걸려 있다"와
"cron 이 의도한 시각에 돈다"는 다른 문제다 — 로그의 실제 실행 시각을 봐야 안다.
