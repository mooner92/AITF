...
3. **이번 주 / 주차별 준비** — `COURSE_START` 기준 주차 계산(휴강 주는 `scripts/term_calendar.py`
   반영, 아래 결정 기록 참조).
...
- [x] 자동 위키(build-wiki.py) 최근 실행 시각으로 표시 전환 완료
...
- 2026-08-29 · **주차 계산을 순수 날짜 나눗셈에서 휴일 인지 방식으로 교체**
  (`scripts/term_calendar.py`, 설정은 `/opt/scripts/term-calendar.json`). `(오늘 - COURSE_START)
  // 7 + 1` 방식은 추석 등 휴강 주를 몰라 그 이후 모든 주차 번호가 밀린다. 명시적 수업일
  목록으로 계산하고, 설정이 없으면 기존 날짜 나눗셈으로 폴백한다. `build-wiki.py`·
  `notify-slack.py`·`publish-notion.py` 세 스크립트에 동일하게 반영. 2026년 추석
  (2026-09-27) 휴강만 확정 상태이고, 다른 휴강일은 미확정.
